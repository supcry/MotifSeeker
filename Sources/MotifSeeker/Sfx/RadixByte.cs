...
namespace MotifSeeker.Sfx
{
	...
	public class RadixByte
	{
		...
		public static unsafe void RadixSortB(ref uint[] sorted, uint[] labels, uint maxLabel, int depth = 0)
		{
			...
			fixed (uint* pRst = resSuftab)
			{
				var pSuftab = pSt;
				var pResSuftab = pRst;
				RadixPass(pSuftab, pResSuftab, pLabels, depth, length);
			}
			sorted = resSuftab;
		}
		...
		public static void RadixSortPairB(ref uint[] sorted, uint[] labels, uint maxLabel, int depth)
		{
			RadixSortB(ref sorted, labels, maxLabel, depth);
			RadixSortB(ref sorted, labels, maxLabel);
		}
		...
		private static unsafe void RadixPassB(uint* sorted, uint* resSorted, uint* labels, int shift, int length)
		{
			var bytes = (byte*)labels;
		...
		private static uint ReLabellingB(ref uint[] labels, ref uint[] tempLabels, uint[] sorted, int p)
		{
			var length = sorted.Length;
			uint label = 0;
			tempLabels[sorted[0]] = label;
